...
function get_tags()
        local t = {}
        local s
        local tags = get_conf("tags") or {}
        for s in wmixp:idir ("/tag") do
                if s.name and not (s.name == "sel") and not tags[s] then
                        t[#t + 1] = s.name
                end
        end
        table.sort(t)
...
function set_view_index(sel)
        ...
        --if #all < 2 then
                -- nothing to do if we have less then 3 tags
                --return
        --end
...
function create_tag_widget(tag)
        local nc = get_ctl("normcolors") or ""
        local screens = get_screens()
        local name = get_tag_widget(tag)
        if not screens then
                create ("/lbar/" .. name, nc .. " " .. tag)
                return
        end
        local i, s
        for i,s in pairs(screens) do
                create ("/screen/"..s.."/lbar/" .. name, nc .. " " .. tag)
        end
end

function destroy_tag_widget(tag)
        local screens = get_screens()
        local name = get_tag_widget(tag)
        if not screens then
                remove ("/lbar/" .. name)
...
local tagidx = nil
function get_tag_widget(tag)
    if not tagidx and get_conf("tags") then
        tagidx = {}
        for i, t in ipairs(get_conf("tags")) do
            tagidx[t] = i
        end
    end
    local idx = tagidx[tag]
    local name = tag
    if idx then
        name = idx..name
    end
