...
Locates the wmiirc script.  It looks in ~/.wmii-lua and /etc/wmii-lua/
...
function tag_display(tag, focused)
        local colors = ""
        if focused then
                colors = get_screen_ctl(scrn, "focuscolors") or get_ctl("focuscolors") or ""
                colors = colors.." "
        else
                colors = get_screen_ctl(scrn, "normcolors") or get_ctl("normcolors") or ""
                colors = colors.." "
        end
...
function update_displayed_tags_on_screen(s)
        ...
        for i,v in pairs(all) do
                ...
                local display = tag_display(v, selected)
                if not old[v] then
                       create (lbar .. "/" .. v, display)
                end
                write (lbar .. "/" .. v, display)
                old[v] = nil
        end
...
function create_tag_widget(name)
        local screens = get_screens()
        local display = tag_display(name)
        if not screens then
                create ("/lbar/" .. name,display)
                return
        end
        local i, s
        for i,s in pairs(screens) do
                create ("/screen/"..s.."/lbar/" .. name, display)
        end
end
...
local ev_handlers = {
	...
	RightBarClick = _handle_widget_event,
        ...
        FocusTag = function (ev, arg)
                ...
                local display = tag_display(tag, true)
                log ("# echo " .. display .. " | wmiir write " .. file)
...
function widget:show (txt, colors)
        local colors = colors or get_conf(self.name..".colors") or get_ctl("normcolors") or ""
